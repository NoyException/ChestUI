...
public class InventoryMonitor implements CUIHandler {
    ...
    public void onInitialize(ChestUI<?> cui) {
        this.displayPlayers = new Layer(5, 9).edit().
                marginTop(1).
                finish();
        this.monitorInventory = new Layer(5, 9).edit().
                marginTop(1).
                editRow(3, slotHandler -> slotHandler.button(builder -> builder.
                        material(Material.WHITE_STAINED_GLASS_PANE).
                        displayName(" ").
                        build())).
                finish();
        this.cui = cui.edit().
                setLayer(0, new Layer(1, 9).edit().
                        ...
                        editSlot(0, 8, slotHandler -> slotHandler.button(builder -> builder.
                                ...
                                build())).
                        finish()).
                setLayer(1, displayPlayers).
                setLayer(3, monitorInventory).
                setActive(monitorInventory, false).
                finish();
    ...
    private void updatePlayers() {
        ...
        for (int row = 0; row < 5; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                displayPlayers.edit().editSlot(row, col, slotHandler -> slotHandler.button(builder -> builder.
                        skull(player).
                        displayName(player.displayName()).
                        clickHandler(event -> monitor(player)).
                        build()));
            }
        }
    }
    ...
    private void monitor(Player player) {
        ...
        for (int i = 0; i < 9; i++) {
            // hotbar, 0~8
            int finalI = i;
            monitorInventory.edit().editSlot(4, i, slotHandler ->
                    slotHandler.storage(builder -> builder.
                            source(inventory, finalI).
                            build()));
        }
        for (int i = 0; i < 27; i++) {
            // main inventory, 9~35
            int finalI = i;
            monitorInventory.edit().editSlot(i / 9, i % 9, slotHandler ->
                    slotHandler.storage(builder -> builder.
                            source(inventory, finalI + 9).
                            build()));
        }

        cui.edit().setActive(0, false).
                setActive(displayPlayers, false).
                setActive(monitorInventory, true).
                finish();
    }
